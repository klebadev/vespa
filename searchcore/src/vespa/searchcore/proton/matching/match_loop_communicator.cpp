...
MatchLoopCommunicator::CompleteSecondPhase::mingle()
{
    ...
    for (size_t i = 0; i < size(); ++i) {
        for (const auto &[hit, tag]: in(i)) {
            ...
            if (new_scores.isValid()) {
                new_scores.low = std::min(new_scores.low, hit.second);
                new_scores.high = std::max(new_scores.high, hit.second);
            } else {
                new_scores = Range(hit.second, hit.second);
            }
        }
    }
